...
function populate_meta_fields() {
	var cat = '<?php echo esc_html($category->slug); ?>';
	var city = '<?php echo esc_html($city); ?>';
	if(cat != '') document.querySelector("#category").getElementsByTagName("select")[0].value = cat;
	if(city != '') document.querySelector("#locality-name").getElementsByTagName("select")[0].value = city;
}
