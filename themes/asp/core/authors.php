...
//AVATAR UPLOAD HANDLER  
  ...
  function updateavatar() {
	...
	$data = array_merge( $posted_data, $file_data );
		...
		$curavatar = '/var/www/new.asp.sale/wp-content/uploads/avatars/'.wp_get_current_user()->ID;
		if ( file_exists( $curavatar.'-300x300.jpg')) { 	
	    
		$attachid = asp_get_image_id(str_replace('/var/www/new.','https://media.',$curavatar.'.jpg'));
		...
		$attachid = asp_get_image_id(str_replace('/var/www/new.','https://media.',$curavatar.'.png'));
		wp_delete_attachment( $attachid, true );	
		...
		$attachid = asp_get_image_id(str_replace('/var/www/new.','https://media.',$curavatar.'.jpeg'));
		wp_delete_attachment( $attachid, true );
...
//UPDATE AVATAR AND DELETE UNUSED SIZES	 
	 ...
	 $r_thumb = str_replace( 'https://media.', '/var/www/new.', $thumb );
	 $r_medium = str_replace( 'https://media.', '/var/www/new.', $medium );
	 $r_large = str_replace( 'https://media.', '/var/www/new.', $large );
	 $r_full = str_replace( 'https://media.', '/var/www/new.', $full );
...
function pbg_avatar( $avatar, $id_or_email, $size ) {
    ...
    if ( $user && is_object( $user ) ) {
	    ...
	    $curavatar = '/var/www/new.asp.sale/wp-content/uploads/avatars/'.$user->data->ID;
