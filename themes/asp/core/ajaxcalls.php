...
function update_blog_ajax() {
    ...
    case 'category':
		...
		wp_set_object_terms($id, array($val), 'category', false);
		$response['response'] = 'success';
...
function update_props_ajax() {
    ...
    if (strlen($val) > 300) exit_ajax('Максимум 300 символов');
	$id = $_POST['id'];
	$val = strip_tags($_POST['metavalue']);
	$meta = strip_tags($_POST['metaname']);
...
if ($sort == 'price') {
	...
	if (has_term(16,'property-type')) $out .= '/сутки'; if (has_term(19,'property-type')) $out .= '/месяц';
	$out .= '</div>';
	}
...
if ($status != 'empty') {
	...
	if ($author == '0') { $out .= '<div class="catprop">'.get_the_author_meta( "display_name" ).'</div>'; };
	$out .= '</div></a>';
